...
def _download_text(url: str, timeout: int = 60) -> str:
    response = get_response(url, timeout=(10, timeout), session=_SESSION)
    text = response.text
    ...
    if text.lstrip().lower().startswith("<!doctype") or "<html" in text[:500].lower():
        ...
        response = get_response(next_url, timeout=(10, timeout), session=_SESSION)
        text = response.text
...
def bindingdb_to_long(df: pd.DataFrame) -> pd.DataFrame:
    ...
    endpoint_columns = {
        endpoint: f"{endpoint} (nM)" for endpoint in BIOACTIVITY_ENDPOINTS if f"{endpoint} (nM)" in df.columns
    }
    ...
    return pd.DataFrame(
        {
            ...
            "date_provenance": "bindingdb_publication_date",
